...
class KurumsalBilgiViewModel : ViewModel() {

    private val apiServis = ApiServis()

    private val disposable = CompositeDisposable()

    val kurumsalBilgi = MutableLiveData<KurumsalBilgiCevap>()
    ...
    fun kurumsalBilgiGetir(url: String) {
        servisYukleniyor.value = true


        disposable.add(
            apiServis.get024KurumsalBilgiX(url, BuildConfig.Pst_CAKKey)
                .subscribeOn(Schedulers.newThread())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeWith(object : DisposableSingleObserver<KurumsalBilgiCevap>() {
                    override fun onSuccess(t: KurumsalBilgiCevap) {
                        kurumsalBilgi.value = t
