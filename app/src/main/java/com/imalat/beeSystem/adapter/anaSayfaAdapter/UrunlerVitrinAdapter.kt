...
class UrunlerVitrinAdapter(
    val list: ArrayList<UrunVitrin>,
    val urunlerGuncelleInterface: UrunlerGuncelleInterface
) :
    RecyclerView.Adapter<UrunlerVitrinAdapter.ViewHpolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHpolder, position: Int) {
        ...
        if (Fonk.doubleCevir(list[position].sepettekiAdet) > 0.0) {
            ...
            //holder.view.frameLayout.visibility = View.VISIBLE
        } else {
            holder.view.cardViewSepeteEkle.visibility = View.VISIBLE
            // holder.view.frameLayout.visibility = View.INVISIBLE
        ...
        holder.view.setOnClickListener {
            ...
            /*
          bu şekilde yaptığımızda geri döndüğümüzde bütün liste baştan tekrar indiriliyor

            val f = UrunDetaylariFrgament ()
            // Pass index input as an argument.
            val args = Bundle()
            args.putString("urunID", list[position].urunID)
            f.setArguments(args)
                ...
                if (Fonk.degerGetir(holder.view.context, EnumX.OturumKodu.toString())
                  ...
                  //  builder.setMessage("En uygun fiyatlarla alışveriş yapmak ve kampanyalardan faydalanmak için hemen ücretsiz üye olun")
                    ...
                    if (maksSipMiktar > sepettekiAdet) {
                        ...
                        holder.view.tvAdet.setText(Fonk.artisMikatriParcala(yeniMiktar.toString()) + " " + list[position].birimAd)
        ...
        holder.view.imAlisVerisListesi.setOnClickListener {
            ...
            if (!Fonk.degerGetir(holder.view.context, EnumX.OturumKodu.toString())
                ...
                if (list[position].favori != null && list[position].favori.equals("1")) {
                    ...
                    // holder.view.imAlisVerisListesi.setImageResource(R.drawable.ic_baseline_playlist_add_24)
                }
                notifyDataSetChanged()
            } else {
    ...
    fun update(newList: List<UrunVitrin>) {
        list.clear()
        list.addAll(newList)
        notifyDataSetChanged()
    }
